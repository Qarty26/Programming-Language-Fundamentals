...
substaexp (Nu x) _ _ = Nu x
substaexp (Qid x) var expr = if x == var then expr else Qid x
substaexp (PlusE a b) var expr = PlusE (substaexp a var expr) (substaexp b var expr)
substaexp (TimesE a b) var expr = TimesE (substaexp a var expr) (substaexp b var expr)
substaexp (DivE a b) var expr = DivE (substaexp a var expr) (substaexp b var expr)
...
valueassn (DisjInfX l) s = or [valueassn a s | a <- l] 
...
substassn (LEX e1 e2) var replacement = LEX (substaexp e1 var replacement) (substaexp e2 var replacement)
substassn (NotEX a) var replacement = NotEX (substassn a var replacement)
substassn (AndEX a1 a2) var replacement = AndEX (substassn a1 var replacement) (substassn a2 var replacement)
substassn (DisjInfX as) var replacement = DisjInfX (map (\a -> substassn a var replacement) as)
...
wp :: Stmt -> Assn -> Assn
wp Skip x = x
wp (AtrE var exp) x = substassn x var exp
wp (Seq s1 s2) x = wp s1 (wp s2 x)
wp (IfE be s1 s2) x = orx (AndEX (convassn be)(wp s1 x))(AndEX (convassn be)(wp s2 x))
wp (WhileE be stmt) as = DisjInfX [p i | i <- [0..]] 
    where
        p :: Int -> Assn
        p 0 = AndEX (convassn (NotE be)) as
        p k = AndEX (convassn be) (wp stmt (p (k-1)) )
