...
evenoddf2 :: Peano -> (Bool, Bool)
evenoddf2 = fix(\f p -> 
            case p of   
                Zero -> (True, False)
                Succ p' -> (snd(f p'), fst(f p')))

testevenoddf = evenoddf (Succ (Succ (Succ Zero))) == (False, True)
testevenoddf2 = evenoddf (Succ(Succ (Succ (Succ Zero)))) == (True, False)
