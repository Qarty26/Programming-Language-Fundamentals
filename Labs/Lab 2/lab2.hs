...
recall s [] = 0
recall s (x:xs) = if fst x == s
                    then snd x
                    else recall s xs

update :: String -> Int -> [(String, Int)] -> [(String, Int)]
update v val [] = [(v,val)]
update v val ((v',val'):rest) = if v == v' then (v,val) : rest else (v',val') : update v val rest
...
value (Nu x) _ = x
value (Qid x) l = recall x l
value (PlusE left right) l = value left l + value right l 
value (TimesE left right) l = value left l * value right l 
value (DivE left right) l = value left l `div` value right l 


valueb :: BExp -> [(String, Int)] -> Bool
valueb (BE x) _  = x
valueb (LE e1 e2) s = (value e1 s) <= (value e2 s) 
valueb (NotE x) s = not (valueb x s)
